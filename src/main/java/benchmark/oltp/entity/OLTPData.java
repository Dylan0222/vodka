...
public class OLTPData {
    ...
    private void executeReceiveGoods(Logger log, OLTPConnection db, BasicRandom rnd) throws Exception {
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    receiveGoods.ol_receipdate = receiveGoods.ol_delivery_d.getTime()
                            + 86400000 * ((long) rnd.nextDouble(1, 30)); // rnd.nextDouble(1, 30)
                    int randomValue = rnd.nextInt(1, 3); // Generates a random integer from 1 to 3
                    if (randomValue == 1)
                    ...
                    try {
                        ...
                        stmt2.executeUpdate();
                        // log.info("Update Comment Execution Time: " + (endTime - startTime) + "ms");
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                }
        ...
        if (htapCheck != null && currentTime != null) {
            final long interval = htapCheck.info.htapCheckFreshnessDataBound * 1000;
            final long elapsedTime = currentTime.getTime() - sessionStartTimestamp;
            final long expectedTime = interval * DeliveryTracker.executionTimes.get();
            final Timestamp finalCurrentTime = new Timestamp(currentTime.getTime()); // 创建一个不可变的时间快照

            // 使用CompletableFuture异步处理数据库写入操作
            CompletableFuture<Void> allWrites = CompletableFuture.allOf(
                    pkLists.stream()
                            .map(item -> CompletableFuture.runAsync(() -> transactionTraceMap.addTransaction(item,
                                    "ol_receipdate", 3, finalCurrentTime)))
                            .toArray(CompletableFuture[]::new));
            if (elapsedTime > expectedTime && htapCheck.info.htapCheckType == HTAPCheckType.BATCH_QUERY
                    && htapCheck.needSpawn()) {
                FreshnessTask2 task = new FreshnessTask2(dbType, htapCheck.info, finalCurrentTime);
                // SynchronizeTask task = new SynchronizeTask(dbType, htapCheck.info,
                // finalCurrentTime);
                htapCheck.trySpawn(task);
                // DeliveryTracker.executionTimes.incrementAndGet(); // Increment after spawning
            }
        }
    }
    ...
    private void executeNewOrder(Logger log, OLTPConnection db, BasicRandom rnd, Date startDate) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < ol_cnt; i++) {
                ...
                insertOrderLineBatch.setTimestamp(14, new Timestamp(System.currentTimeMillis()));
        ...
        if (OLTPClient.isSampling) {
            CompletableFuture.runAsync(() -> {
                writeOrders.forEach(order -> OLTPClient.sampler4Order.add(order));
            });
        }
    ...
    private void executeDeliveryBG(Logger log, OLTPConnection db, BasicRandom rnd) throws Exception {
        ...
        try {
            for (d_id = 1; d_id <= 10; d_id++) {
                ...
                // log.info("Update Order Execution Time: " + (endTime - startTime) + "ms");
                ...
                // int tmpDays2 = (int) ((o_entry_d.getTime() - currentDate2.getTime()) / (1000
                // * 60 * 60 * 24));
                ...
                stmt1.setInt(1, deliveryBG.w_id);
                // System.out.println(deliveryBG.w_id);
                ...
                currentTime = new Timestamp(System.currentTimeMillis());
                // taskTrack.logTransaction(deliveryBG.w_id, d_id, o_id, currentTime);
                ...
                if (htapCheck != null) {
                    List<Integer> pkList = Arrays.asList(deliveryBG.w_id, d_id, o_id);
                    pkLists.add(pkList);
                }
                // log.info("Update Order Line Execution Time: " + (endTime - startTime) +
                // "ms");
                ...
                rs = stmt1.executeQuery();
                endTime = System.currentTimeMillis();
                // log.info("Select Sum OL Amount Execution Time: " + (endTime - startTime) +
                // "ms");
                ...
                stmt1.executeUpdate();
                endTime = System.currentTimeMillis();
                // log.info("Update Customer Execution Time: " + (endTime - startTime) + "ms");
            ...
            if (htapCheck != null) {
                final Timestamp finalCurrentTime = new Timestamp(endTime);
                CompletableFuture<Void> allWrites = CompletableFuture.allOf(
                        pkLists.stream()
                                .map(item -> CompletableFuture.runAsync(() -> transactionTraceMap.addTransaction(item,
                                        "ol_delivery_d", 2, finalCurrentTime)))
                                .toArray(CompletableFuture[]::new));
            }

            // if (htapCheck != null) {
            // long interval = htapCheck.info.htapCheckFreshnessDataBound * 1000;
            // long elapsedTime = currentTime.getTime() - sessionStartTimestamp;
            // long expectedTime = interval * DeliveryTracker.executionTimes.get();
            // // System.out.println(elapsedTime + "," + expectedTime);

            // if (elapsedTime > expectedTime && htapCheck.info.htapCheckType ==
            // HTAPCheckType.BATCH_QUERY
            // && htapCheck.needSpawn()) {
            // FreshnessTask task = new FreshnessTask(dbType, htapCheck.info, currentTime);
            // htapCheck.trySpawn(task);
            // DeliveryTracker.executionTimes.incrementAndGet(); // Increment after spawning
            // the task
            // }
    ...
    // private void executeDeliveryBG(Logger log, OLTPConnection db, BasicRandom
    // rnd) throws Exception {
    // PreparedStatement stmt1;
    // PreparedStatement stmt2;
    // ResultSet rs;
    // int rc;
    ...
    //// stmt1.setTimestamp(1, new java.sql.Timestamp(now));
    // // current date
    // stmt1.setTimestamp(1, new Timestamp(o_entry_d.getTime() + 86400000L *
    // rnd.nextInt(10, 10)));//rnd.nextInt(1, 121)
    // stmt1.setString(2, rnd.getSmode(0, 6)); //Update ORDER_LINE Set Smode
    // stmt1.setString(3, rnd.getSinstruct(0, 3)); // Update ORDER_LINE Set
    // sInstruct
    // stmt1.setInt(4, deliveryBG.w_id);
    // stmt1.setInt(5, d_id);
    // stmt1.setInt(6, o_id);
    ...
    // }
    //
    // if (htapCheck != null && htapCheck.info.htapCheckType ==
    // HTAPCheckType.AD_HOC_UPDATE && htapCheck.needSpawn()) {
    // AdHocUpdateTask task = new AdHocUpdateTask(task_o_id, task_d_id,
    // deliveryBG.w_id, htapCheck.info.gapTime);
    // htapCheck.trySpawn(task);
    // }
    // if (htapCheck != null && htapCheck.info.htapCheckType ==
    // HTAPCheckType.BATCH_QUERY && htapCheck.needSpawn()) {
    // BatchQueryTask task = new BatchQueryTask(deliveryBG.w_id,
    // htapCheck.getRandomDid(), htapCheck.info.gapTime);
    // htapCheck.trySpawn(task);
    // }
    // } catch (SQLException se) {
    ...
    private void executeDeliveryBGTiDB(Logger log, OLTPConnection db, BasicRandom rnd) throws Exception {
        ...
        try {
            ...
            for (d_id = 1; d_id <= 10; d_id++) {
                ...
                stmt1.executeUpdate();
                // if (htapCheck != null) {
                // Pair<Long, Long> timePair = new Pair<>(System.currentTimeMillis(),
                // delivery_d);
                // OLTPClient.deliveryList.add(timePair);
                // }
            }
            ...
            if (htapCheck != null && htapCheck.info.htapCheckType == HTAPCheckType.AD_HOC_UPDATE
                    && htapCheck.needSpawn()) {
                AdHocUpdateTask task = new AdHocUpdateTask(task_o_id, task_d_id, deliveryBG.w_id,
                        htapCheck.info.gapTime);
                htapCheck.trySpawn(task);
            }
            if (htapCheck != null && htapCheck.info.htapCheckType == HTAPCheckType.BATCH_QUERY
                    && htapCheck.needSpawn()) {
                BatchQueryTask task = new BatchQueryTask(task_o_id, task_d_id, deliveryBG.w_id, delivery_d, dbType,
                        htapCheck.info, System.currentTimeMillis());
                htapCheck.trySpawn(task);
            }
        } catch (SQLException se) {
            try {
