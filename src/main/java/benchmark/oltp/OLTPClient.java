...
public class OLTPClient implements CommonConfig {
    ...
    public static ReservoirSampler<OrderLine> sampler4OrderLine = new ReservoirSampler<>(5000000);
    ...
    public OLTPClient() throws ParseException, IOException {
        ...
        boolean isSampling = runningProperty.isSampling();
        OLTPClient.isSampling = isSampling;
        ...
        if (databaseDriverLoaded) {
            try {
                ...
                MemoryMonitor memoryMonitor = new MemoryMonitor(resultDirName);
                ...
                try {
                    ...
                    for (int i = 0; i < numTerminals; i++) {
                        ...
                        OLTPTerminal terminal = new OLTPTerminal(terminalName, terminalWarehouseID, terminalDistrictID,
                                conn, dbType,
                    ...
                    TableInfoCollector infoCollector = new TableInfoCollector(iConn, dbProps, dbType, (int) limPerMin,
                            newOrderWeightValue, paymentWeightValue, deliveryWeightValue, receiveGoodsWeightValue);
                    ...
                    if (checkInterference) {
                        DynamicAP dynamicAP = new DynamicAP(iConn, dbProps, dbType, testTimeInterval, this,
                                dynamicParam, parallelSwitch, isolation_level, parallel_degree, resultDirName, step,
                                increaseInterval);
    ...
    public void prepareForLog(String resultDirectory, String osCollectorScript, int osCollectorInterval,
        ...
        if (databaseDriverLoaded && resultDirectory != null) {
            ...
            if (osCollectorScript != null) {
                osCollector = new OSCollector(osCollectorScript, runID, osCollectorInterval, osCollectorSSHAddr,
