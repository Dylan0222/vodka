...
public class BatchQueryTask extends Task {
    ...
    public BatchQueryTask(int o_id, int d_id, int w_id, long ol_delivery_d, int dbType, HTAPCheckInfo htapCheckInfo,
        ...
        batchQuerySql = "SELECT /*+READ_CONSISTENCY(WEAK) */ sum(vodka_order_line.access_version)" +
                "   FROM vodka_order_line, vodka_oorder " +
                "   WHERE " +
                " ol_w_id = o_w_id and ol_d_id = o_d_id and ol_o_id = o_id and" +
                " ol_w_id >= ? AND" +
    ...
    public TaskResult runTask(ArrayList<Connection> conns, int threadId) {
        ...
        try {
            ...
            if (isWeakRead) {
                if (dbType == CommonConfig.DB_OCEANBASE) {
                    ...
                    weakStatement.execute("set _force_parallel_query_dop = 4");
                }
            }
            if (!resultMatch) {
                ...
                if (resultSetAP.next()) {
                    ...
                    System.out.printf("[BATCH_QUERY] [%d] version in AP is [%d], and version in TP is [%d]",
        ...
        try {
            ...
            Statement weakStatement = conns.get(0).createStatement();
            if (this.isWeakRead && dbType == CommonConfig.DB_OCEANBASE) {
                weakStatement.execute("SET ob_read_consistency = WEAK;");
            }
            startTime = System.nanoTime();
            ...
            txnCompleteTime = System.nanoTime();
            if (result.next()) {
                System.out.println("version is:" + result.getString(1));
            }
            ...
            // Triple<Long, Long, Long> freshnessItem = new Triple<Long, Long, Long>(freshness, latency, tmpLatency);
