...
SELECT CASE count(*) WHEN 0 THEN 'OK   ' ELSE 'ERROR' END AS "check",
	...
	SELECT o_w_id, o_d_id, o_id, o_ol_cnt, count(*) AS "actual"
	    FROM vodka_oorder
	    LEFT JOIN vodka_order_line ON ol_w_id = o_w_id AND ol_d_id = o_d_id
		    AND ol_o_id = o_id
...
SELECT CASE count(*) WHEN 0 THEN 'OK   ' ELSE 'ERROR' END AS "check",
	...
	SELECT w_id, w_ytd, sum(d_ytd) AS sum_d_ytd
	    ...
	    LEFT JOIN vodka_district ON d_w_id = w_id
	    GROUP BY w_id, w_ytd
...
SELECT CASE count(*) WHEN 0 THEN 'OK   ' ELSE 'ERROR' END AS "check",
	...
	SELECT sum_w_ytd, sum_c_ytd_payment
	    FROM (SELECT sum(w_ytd) AS sum_w_ytd FROM vodka_warehouse) AS W,
	         (SELECT sum(c_ytd_payment) AS sum_c_ytd_payment FROM vodka_customer) AS C
...
SELECT CASE count(*) WHEN 0 THEN 'OK   ' ELSE 'ERROR' END AS "check",
	...
	SELECT c_w_id, c_d_id, c_id, coalesce(sum_ol_amount, 0.0) AS sum_ol_amount,
		...
		SELECT o_w_id, o_d_id, o_c_id, sum(ol_amount) as sum_ol_amount
		    FROM vodka_oorder
		    JOIN vodka_order_line ON ol_w_id = o_w_id AND ol_d_id = o_d_id AND ol_o_id = o_id
